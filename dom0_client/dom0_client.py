...
class Dom0_session:
	...
	def pause(self):
		print('Pausing tasks on server.')
		meta = struct.pack('I', magicnumbers.PAUSE)
		self.conn.send(meta)

	def resume(self):
		print('Resuming tasks on server.')
		meta = struct.pack('I', magicnumbers.RESUME)
		self.conn.send(meta)
...
Available commands:
	...
	session.pause()				: Pause tasks on server.
	session.resume()			: Resume tasks on server.
